...
#include "solace/OS.h"
#include "solace/solClient.h"
#include "solace/solClientMsg.h"
#include <fstream>
#include <iostream>
...
{
    std::ifstream fileP;
    fileP.open(fileName, std::ios::in);
    if (!fileP)
    {
...
int main ( int argc, char *argv[] )
{
    ...
    if (argc < 6) {
        printf("Usage: QueuePublisher <msg_backbone_ip:port> <vpn> <client-username> <password> <queue-name> <FILE/TEXT> <file-name>\n");
        return -1;
    }

    std::string fileContent = "Sending default message";

    if (strcmp(argv[6], "FILE") == 0)
        fileContent = readFromFile(argv[7]);
    else
        fileContent = argv[7];
    ...
    solClient_msg_setBinaryAttachment ( msg_p, fileContent.c_str(), ( solClient_uint32_t ) strlen ( (char *)fileContent.c_str()) );

    /* Send the message. */
    printf ( "About to send message '%s' to queue '%s'...\n", (char *)fileContent.c_str(), argv[5] );
